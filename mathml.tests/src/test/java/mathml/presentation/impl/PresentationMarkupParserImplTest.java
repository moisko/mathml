...
public class PresentationMarkupParserImplTest extends MathMLBaseTest {
	...
	private static final String PRESENTATION_MARKUP_DIR = MathMLBaseTest.MATHML_REPOSITORY_DIR
			+ File.separator + PRESENTATION_MARKUP_DIR_NAME;
	...
	public void testParseMathFile_1() throws Exception {
		...
		IMathMLExpression actualExpression = parser.parse(new FileInputStream(
				pMathFile_1));
		IMathMLExpression expectedExpression = new Expression(
				expectedExpression_1.getExpressionOperands());
		...
		MathMLTransformerFactory factory = MathMLTransformerFactory
				.getDefault();
		MathMLTransformer transformer = factory.createMathMLTransformer();
		transformer.transform(actualExpression, new OutputStreamWriter(
				System.out));
	}
	...
	public void testParseMathFile_2() throws Exception {
		...
		IMathMLExpression actualExpression = parser.parse(new FileInputStream(
				pMathFile_2));
		IMathMLExpression expectedExpression = new Expression(
				expectedExpression_2.getExpressionOperands());
		...
		// MathMLTransformerFactory factory =
		// MathMLTransformerFactory.newMathMLTransformerFactory();
		// MathMLTransformer transformer = factory.newMathMLTransformer();
		// transformer.transform(actualExpression, new
	...
	//
	// List<PresentationMarkupElement> parsedMathMLElements = parser.parse(new
	// FileInputStream(pMathFile_3));
	//
	...
	// // TODO check the case when there is no <mrow> tag element after the
	// <math> root tag element
